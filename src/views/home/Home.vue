<template>
  <div class="home">
    <div class="loading">CODEJAY</div>
    <ul class="home_nav">
      <li v-for="item in menu" @click="itemClick(item.path)">
        <i class="iconfont" v-html="item.iconUrl"></i>
        {{item.name}}
      </li>
    </ul>
    <ul class="home_link">
      <li v-for="item in link">{{item}}</li>
    </ul>
  </div>
</template>

<script>
import { blogName } from "./childComps/demo";
export default {
  name: "Home",
  data() {
    return {
